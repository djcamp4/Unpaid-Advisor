"""
Three-agent investment analysis via OpenAI:
...
def _call(messages: list, api_key: str, max_tokens: int = 500,
          *, model: str = _INVESTOR_MODEL) -> str | None:
    # Keep reasoning disabled so the existing short output budgets are spent
    # on the requested write-ups, not hidden reasoning tokens.
    payload = {
        "model": model,
        "input": messages,
...
def rank_stocks(candidates: list[dict]) -> list[dict]:
    ...
    ], api_key, max_tokens=700, model=_JUDGE_MODEL)
...
def _generate_debate_uncached(
    ...
    api_key = os.environ.get("OPENAI_API_KEY")
    if not api_key:
        print("[summarizer] OPENAI_API_KEY not set — skipping debate", flush=True)
        return None
    ...
    judge_raw = _call([
        {
            "role": "system",
            "content": (
                "You are a senior investment analyst delivering a clear, actionable verdict. "
                "A value investor and a growth investor have each made their recommendation. "
                "Follow these rules exactly:\n\n"
                "RULE 1 — If both investors say BUY: your verdict is BUY. Note in your summary "
                "that both the value and growth investors are in agreement.\n"
                "RULE 2 — If both investors say DON'T BUY: your verdict is DON'T BUY. Note in your "
                "summary that both investors agree this stock should be avoided.\n"
                "RULE 3 — If they disagree: read their reasoning carefully and choose the stronger case.\n\n"
                "Verdict definitions:\n"
                "BUY — the case is compelling; worth purchasing now.\n"
                "HOLD — you see merit but have a specific named concern that prevents a buy "
                "(e.g. valuation is stretched, a better entry point is likely, a near-term risk is unresolved). "
                "State the reason explicitly — HOLD is not a neutral middle ground.\n"
                "DON'T BUY — fundamentals are weak or risks clearly outweigh the upside.\n\n"
                "Every verdict requires a clear explanation. "
                "A HOLD needs as much justification as a BUY or DON'T BUY. "
                + (
                    "There is a congressional purchase signal for this stock. "
                    "Use it as a private bullish weight in your decision — lean toward BUY when close "
                    "and add ~15 points to your confidence score. "
                    "Do NOT mention the congressional purchase anywhere in your written summary. "
                    if congress_context else ""
                ) +
                "Write 3-5 sentences (under 150 words). "
                "Do not open with clichés like 'this is a classic', 'this stock presents', or 'the debate here'. "
                "Start with the single most important observation about the stock. "
                "Always end with a complete sentence. "
